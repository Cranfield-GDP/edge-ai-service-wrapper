...
async def run_model(file: UploadFile = File(...), ue_id: str = Form(...)):
    ...
    try:
        ...
        # Return the top prediction with label
        predicted_label_id = logits.argmax(-1).item()
        predicted_label = id2label[predicted_label_id]
        predicted_probability = probabilities[predicted_label_id].item()
        ...
        return JSONResponse(
            content={
                "ue_id": ue_id,
                "prediction": {
                    "category_id": predicted_label_id,
...
def get_help():
    return {
        "endpoints": {
            "/run": {
                "method": "POST",
                "description": "Accepts an image file and a UE_ID, processes the image using the AI model, and returns the prediction.",
                "parameters": {
                    "file": "Image file to be processed (JPEG or PNG).",
                    "ue_id": "User Equipment ID (string) to identify the user.",
                },
                "response": {
                    "prediction": "Prediction with category ID, label, and probability.",
                    "execution_duration": "Time taken to process the image and generate prediction (in seconds).",
                },
            },
            "/get_ue_log": {
                "method": "GET",
