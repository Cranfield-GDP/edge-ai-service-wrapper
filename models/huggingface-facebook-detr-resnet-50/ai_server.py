...
def get_help():
    return {
        "endpoints": {
            "/run": {
                "method": "POST",
                "description": "Accepts an image file and a UE_ID, processes the image using the AI model, and returns the predictions.",
                "parameters": {
                    "file": "Image file to be processed (JPEG or PNG).",
                    "ue_id": "User Equipment ID (string) to identify the user.",
                },
                "response": {
                    "predictions": "List of predictions with labels, scores, and bounding boxes.",
