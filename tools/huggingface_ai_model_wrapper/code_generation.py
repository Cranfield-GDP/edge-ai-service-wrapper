...
assert validate_hf_model_name(example_model_name), f"The example model '{example_model_name}' is invalid."

example_model_directory = get_hf_model_directory(example_model_name)
assert os.path.exists(example_model_directory), f"The example model directory '{example_model_directory}' does not exist."
...
for file_name in TARGET_FILES_TO_GENERATE:
    ...
    assert os.path.exists(file_path), f"The example model file '{file_path}' does not exist."
...
# --------------------------------
copy_logger_file(hf_model_directory, example_model_files_content, output_files_content)
